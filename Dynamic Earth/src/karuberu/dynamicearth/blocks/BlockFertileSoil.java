...
public class BlockFertileSoil extends BlockDynamicEarth implements ITextureOverlay, ITillable, ISoil, IGrassyBlock {
    ...
    public void registerIcons(IconRegister iconRegister) {
		this.soil = iconRegister.registerIcon(BlockTexture.FERTILESOIL.getIconPath());
		this.grassSide = iconRegister.registerIcon(BlockTexture.FERTILEGRASSSIDE.getIconPath());
		this.myceliumSide = iconRegister.registerIcon(BlockTexture.FERTILEMYCELIUMSIDE.getIconPath());
		this.snowSide = iconRegister.registerIcon(BlockTexture.FERTILESNOWSIDE.getIconPath());
		...
		if (!world.isRemote) {
			...
			if (metadata != SOIL) {
				...
				&& world.getBlockLightOpacity(x, y + 1, z) > 2) {
		            world.setBlock(x, y, z, DynamicEarth.fertileSoil.blockID, SOIL, MCHelper.NOTIFY_WITHOUT_UPDATE);
				} else {
    ...
    protected ItemStack getWetBlock(int metadata) {
    	switch (metadata) {
    	case SOIL: return fertileMud == null ? fertileMud = new ItemStack(DynamicEarth.mud.blockID, 1, BlockMud.FERTILE) : fertileMud;
    	default: return null;
    	}
    }
	...
	public void tryToGrow(World world, int x, int y, int z, EnumGrassType type) {
		...
		&& this.isLightSufficient(world, x, y, z)) {
			switch (type) {
			case GRASS:
				world.setBlock(x, y, z, DynamicEarth.fertileSoil.blockID, GRASS, MCHelper.NOTIFY_AND_UPDATE_REMOTE);
				break;
			case MYCELIUM:
				world.setBlock(x, y, z, DynamicEarth.fertileSoil.blockID, MYCELIUM, MCHelper.NOTIFY_AND_UPDATE_REMOTE);
				break;
			default:
				return;
			}
	...
	public ItemStack getBlockForType(World world, int x, int y, int z, EnumGrassType type) {
		switch (type) {
		case DIRT:
			return new ItemStack(DynamicEarth.fertileSoil, 1, SOIL);
		case GRASS:
			return new ItemStack(DynamicEarth.fertileSoil, 1, GRASS);
		case MYCELIUM:
			return new ItemStack(DynamicEarth.fertileSoil, 1, MYCELIUM);
		}
		return null;
	}
				...
				if (random.nextInt(50) == 0) {
					if (topBlock instanceof BlockMushroom) {
						...
						if (!((BlockMushroom)topBlock).fertilizeMushroom(world, x, y + 1, z, random)) {
							world.setBlock(x, y, z, DynamicEarth.fertileSoil.blockID, MYCELIUM, MCHelper.DO_NOT_NOTIFY_OR_UPDATE);
						} else {
	...
	public boolean onTilled(World world, int x, int y, int z) {
		...
		case SOIL:
		case GRASS:
			world.setBlock(x, y, z, DynamicEarth.farmland.blockID, BlockDynamicFarmland.SOIL_DRY, MCHelper.NOTIFY_AND_UPDATE_REMOTE);
			return true;
		}
		return false;
	}
    ...
    public int idDropped(int par1, Random par2Random, int par3) {
    	return DynamicEarth.fertileSoil.blockID;
    }
