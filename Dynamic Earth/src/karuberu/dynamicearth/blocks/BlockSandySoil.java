...
public class BlockSandySoil extends Block implements ITextureOverlay, ITillable, IGrassyBlock {
    ...
    public void registerIcons(IconRegister iconRegister) {
		this.dirt = iconRegister.registerIcon(BlockTexture.SANDYSOIL.getIconPath());
		this.grassSide = iconRegister.registerIcon(BlockTexture.SANDYGRASSSIDE.getIconPath());
		this.myceliumSide = iconRegister.registerIcon(BlockTexture.SANDYMYCELIUMSIDE.getIconPath());
		this.snowSide = iconRegister.registerIcon(BlockTexture.SANDYSNOWSIDE.getIconPath());
		...
		if (!world.isRemote) {
			...
			if (metadata != DIRT) {
				...
				&& world.getBlockLightOpacity(x, y + 1, z) > 2) {
		            world.setBlock(x, y, z, DynamicEarth.sandySoil.blockID, DIRT, MCHelper.NOTIFY_WITHOUT_UPDATE);
				}
			}
		}
	...
	public void tryToGrow(World world, int x, int y, int z, EnumGrassType type) {
		...
		&& this.isLightSufficient(world, x, y, z)) {
			switch (type) {
			case GRASS:
				world.setBlock(x, y, z, DynamicEarth.sandySoil.blockID, GRASS, MCHelper.NOTIFY_AND_UPDATE_REMOTE);
				break;
			case MYCELIUM:
				world.setBlock(x, y, z, DynamicEarth.sandySoil.blockID, MYCELIUM, MCHelper.NOTIFY_AND_UPDATE_REMOTE);
				break;
			default:
				return;
			}
	...
	public ItemStack getBlockForType(World world, int x, int y, int z, EnumGrassType type) {
		switch (type) {
		case DIRT:
			return new ItemStack(DynamicEarth.sandySoil, 1, DIRT);
		case GRASS:
			return new ItemStack(DynamicEarth.sandySoil, 1, GRASS);
		case MYCELIUM:
			return new ItemStack(DynamicEarth.sandySoil, 1, MYCELIUM);
		}
		return null;
	}
	...
	public boolean onTilled(World world, int x, int y, int z) {
		...
		case DIRT:
		case GRASS:
			world.setBlock(x, y, z, DynamicEarth.farmland.blockID, BlockDynamicFarmland.SANDY_DRY, MCHelper.NOTIFY_AND_UPDATE_REMOTE);
			return true;
		}
		return false;
	}
    ...
    public void onPlantGrow(World world, int x, int y, int z, int sourceX, int sourceY, int sourceZ) {
		world.setBlock(x, y, z, DynamicEarth.sandySoil.blockID, DIRT, MCHelper.NOTIFY_AND_UPDATE_REMOTE);
	}
    ...
    public int idDropped(int id, Random random, int damage) {
    	return DynamicEarth.sandySoil.blockID;
    }
