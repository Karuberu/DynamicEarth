...
public class BlockMud extends BlockMudMod implements IFallingBlock, INeighborBlockEventHandler, ITillable {
	...
	protected ItemStack getDryBlock(int metadata) {
		...
		case WET: return dryMud == null ? dryMud = new ItemStack(MudMod.mud.blockID, 1, NORMAL) : dryMud;
		case FERTILE: return fertileSoil == null ? fertileSoil = new ItemStack(MudMod.fertileSoil.blockID, 1, BlockFertileSoil.SOIL) : fertileSoil;
		case FERTILE_WET: return fertileMud == null ? fertileMud = new ItemStack(MudMod.mud.blockID, 1, FERTILE) : fertileMud;
		}
		return null;
	}
	...
	protected ItemStack getWetBlock(int metadata) {
		switch (metadata) {
		case NORMAL: return wetMud == null ? wetMud = new ItemStack(MudMod.mud.blockID, 1, WET) : wetMud;
		case FERTILE: return fertileMudWet == null ? fertileMudWet = new ItemStack(MudMod.mud.blockID, 1, FERTILE_WET) : fertileMudWet;
		}
		return null;
	}
	...
	protected ItemStack getBlockForSpread(World world, int x, int y, int z, int targetX, int targetY, int targetZ) {
		...
		if (targetID == Block.dirt.blockID) {
			return new ItemStack(MudMod.mud.blockID, 1, NORMAL);
		} else if (targetID == MudMod.fertileSoil.blockID) {
			return new ItemStack(MudMod.mud.blockID, 1, FERTILE);
		}
		return null;
	}
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
    	...
    	switch(metadata) {
    	case FALLING:
    		world.setBlock(x, y, z, MudMod.mud.blockID, NORMAL, MCHelper.DO_NOT_NOTIFY_OR_UPDATE);
    		break;
    	case FALLING_WET:
    		world.setBlock(x, y, z, MudMod.mud.blockID, WET, MCHelper.DO_NOT_NOTIFY_OR_UPDATE);
    		break;
    	case FALLING_FERTILE:
    		world.setBlock(x, y, z, MudMod.mud.blockID, FERTILE, MCHelper.DO_NOT_NOTIFY_OR_UPDATE);
    		break;
    	case FALLING_FERTILE_WET:
    		world.setBlock(x, y, z, MudMod.mud.blockID, FERTILE_WET, MCHelper.DO_NOT_NOTIFY_OR_UPDATE);
    		break;
    	}
    }
	...
	public boolean onTilled(World world, int x, int y, int z) {
		...
		switch (metadata) {
		case FERTILE:
		case FERTILE_WET: metadata = BlockMudModFarmland.SOIL_WET; break;
		default: metadata = BlockMudModFarmland.MUD; break;
		}
	...
	protected boolean willHydrate(World world, int x, int y, int z) {
		if (BlockMudMod.isBlockGettingRainedOn(world, x, y, z)) {
			return true;
		} else {
