...
public abstract class BlockDynamicEarthWet extends BlockDynamicEarth {
	private int
		hydrateRadiusX,
		hydrateRadiusYUp,
		hydrateRadiusYDown,
	...
	public BlockDynamicEarthWet(String unlocalizedName, Material material) {
	...
	protected final BlockDynamicEarthWet setHydrateRadius(int x, int up, int down, int z) {
		hydrateRadiusX = x;
		hydrateRadiusYUp = up;
	...
	protected final BlockDynamicEarthWet setHydrateRadius(int x, int y, int z) {
		return setHydrateRadius(x, y, y, z);
	}
	protected final BlockDynamicEarthWet setHydrateRadius(int radius) {
		return setHydrateRadius(radius, radius, radius, radius);
	}
	
	protected final BlockDynamicEarthWet setSimpleHydration(boolean bool) {
	...
	protected void spread(World world, int x, int y, int z, int attempts) {
		...
		Block block;
		BlockDynamicEarthWet dynamicBlock;
		for (int i = 0; i < attempts; ++i) {
			...
			if (spreadBlock != null) {
				...
				if (block instanceof BlockDynamicEarthWet) {
					dynamicBlock = (BlockDynamicEarthWet)block;
	...
	public static boolean isBlockGettingRainedOn(World world, int x, int y, int z) {
		...
		if (BlockDynamicEarthWet.isBlockExposedToTheSky(world, x, y, z, BlockSide.TOP, BlockSide.EAST, BlockSide.WEST, BlockSide.NORTH, BlockSide.SOUTH)) {
			return true;
		} else if (DynamicEarth.enableMoreDestructiveRain){
			if (!BlockDynamicEarthWet.isBlockWet(world, x, y + 1, z)) {
				return false;
			} else if (BlockDynamicEarthWet.isBlockExposedToTheSky(world, x, y + 1, z, BlockSide.TOP, BlockSide.EAST, BlockSide.WEST, BlockSide.NORTH, BlockSide.SOUTH)) {
				return true;
			} else {
				return BlockDynamicEarthWet.isBlockWet(world, x, y + 2, z)
				&& BlockDynamicEarthWet.isBlockExposedToTheSky(world, x, y + 2, z, BlockSide.TOP, BlockSide.EAST, BlockSide.WEST, BlockSide.NORTH, BlockSide.SOUTH);
			}
		} else {
			return false;
		}
	...
	public static boolean isBlockWet(World world, int x, int y, int z) {
		...
		int metadata = world.getBlockMetadata(x, y, z);
		if (block instanceof BlockDynamicEarthWet
		&& ((BlockDynamicEarthWet)block).canDry(metadata)) {
			return true;
		}
		return false;
	}
